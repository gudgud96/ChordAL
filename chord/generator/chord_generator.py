...
def generate_chord_from_seed(model, num_of_chords=40):
    ...
    while len(result_chord) < num_of_chords:   # terminating condition now only consist of a fixed length
        ...
        next_chord = generate_next_chord(cur_chord_block, model)
...
def generate_next_chord(chord_block, model):
    predict_chord_oh = one_hot_encode_chords(chord_block)
...
def id_to_chord_test():
    ...
    assert id_to_chord(3) == "C:min"
    assert id_to_chord(14) == "F#:maj"
    assert id_to_chord(6) == "D:maj"
    assert id_to_chord(13) == "F:min"
...
def main():
    # X_train, X_test, Y_train, Y_test = preprocess_data(CHORD_SEQUENCE_FILE, TT_SPLIT)
    if os.path.exists('chord_model.h5'):
    ...
    generate_chord_from_seed(model)
    # chord_to_id_test()
    # id_to_chord_test()
